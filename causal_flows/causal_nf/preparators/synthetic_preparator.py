from causal_nf.datasets.synthetic import SyntheticDataset
...
class SyntheticPreparator(TabularPreparator):
    def __init__(self, add_noise, nr_nodes, nr_edges, data_seed, **kwargs):

        self.dataset = None
        self.add_noise = add_noise
        sem_fn = sem_dict["synthetic"](nr_nodes, nr_edges, data_seed, sem_name="dummy")

        self.adjacency = sem_fn.adjacency

        self.num_nodes = nr_nodes
        self.num_edges = nr_edges
        self.data_seed = data_seed

        self.intervention_index_list = sem_fn.intervention_index_list()
        super().__init__(name="synthetic", task="modeling", **kwargs)
    ...
    def params(cls, dataset):
        ...
        my_dict = {
            ...
            "nr_nodes": dataset.nr_nodes,
            "nr_edges": dataset.nr_edges,
    ...
    def loader(cls, dataset):
        my_dict = SyntheticPreparator.params(dataset)
    ...
    def log_prob(self, x):
        ...
        px = Independent(
            Normal(
            torch.zeros(self.num_nodes, device=device),
            torch.ones(self.num_nodes, device=device),
            ),
            1,
        )
    ...
    def _split_dataset(self, dataset_raw):
        ...
        for i, split_s in enumerate(self.split):
            dataset = SyntheticDataset(
                root_dir=self.root, split=self.split_names[i], seed=self.k_fold, num_nodes=self.num_nodes, num_edges=self.num_edges, data_seed=self.data_seed,
            )
