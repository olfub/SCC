...
log_base_dir = environment["experiments"]["base"] / "E4" / "logs"
...
# remove the conf.remove_worlds least frequent worlds from the dataset
# this code assumes that in dataset.X, there is first the intervention information and then the original world variables
assert X_vars[0] == "intervention"
if conf.remove_worlds > 0:
    orig_world_size = len(X_vars) - 1
    worlds = dataset.X[:, -orig_world_size:]
    unique_worlds, counts = torch.unique(worlds, dim=0, return_counts=True)
    sorted_indices = torch.argsort(counts, descending=True)
    sorted_worlds = unique_worlds[sorted_indices]
    worlds_to_remove = sorted_worlds[-conf.remove_worlds:]
    mask = torch.ones(worlds.shape[0], dtype=bool, device=worlds.device)
    for world in worlds_to_remove:
        mask &= ~torch.all(worlds == world, dim=1)
