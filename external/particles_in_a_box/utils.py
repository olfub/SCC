...
def get_init_conditions(
    ...
    radii = np.random.uniform(
        low=radii_limits[0], high=radii_limits[1], size=(n_particles,)
    )
    coordinates = []

    x_first = np.random.uniform(
        low=position_limits_x[0], high=position_limits_x[1], size=(1,)
    )[0]
    y_first = np.random.uniform(
        low=position_limits_y[0], high=position_limits_y[1], size=(1,)
    )[0]
    coordinates.append([x_first, y_first])

    while len(coordinates) < n_particles:
        x_check = np.random.uniform(
            low=position_limits_x[0], high=position_limits_x[1], size=(1,)
        )[0]
        y_check = np.random.uniform(
            low=position_limits_y[0], high=position_limits_y[1], size=(1,)
        )[0]
        coordinates.append([x_check, y_check])
        radii_sum = radii[: len(coordinates), None] + radii[None, : len(coordinates)]
        distance_matrix = np.linalg.norm(
            np.array(coordinates) - np.array(coordinates)[:, None], axis=-1
        )
        overlapping_particles_indicies = np.where(
            (distance_matrix != 0) & (distance_matrix <= radii_sum + 1.0)
        )
    ...
    v_x_init = np.random.uniform(
        low=velocity_limits_x[0], high=velocity_limits_x[1], size=(n_particles,)
    )
    v_y_init = np.random.uniform(
        low=velocity_limits_y[0], high=velocity_limits_y[1], size=(n_particles,)
    )
...
def update_positions(
    particles, x_new, y_new, box_size_x, box_size_y, restitution_coef_bc=1.0
):
    """
    This function detects particle-border colissions and updates particle positions
    """
    # Particle-boundary collisions
    particles.v_x[np.where(x_new + particles.radii > box_size_x / 2)] = (
        -1
        * restitution_coef_bc
        * particles.v_x[np.where(x_new + particles.radii > box_size_x / 2)]
    )
    ...
    particles.v_y[np.where(y_new + particles.radii > box_size_y / 2)] = (
        -1
        * restitution_coef_bc
        * particles.v_y[np.where(y_new + particles.radii > box_size_y / 2)]
    )
    ...
    x_new[np.where(x_new + particles.radii > box_size_x / 2)] = (
        box_size_x / 2
        - particles.radii[np.where(x_new + particles.radii > box_size_x / 2)]
    )
    x_new[np.where(x_new - particles.radii < -box_size_x / 2)] = (
        -box_size_x / 2
        + particles.radii[np.where(x_new - particles.radii < -box_size_x / 2)]
    )
    y_new[np.where(y_new + particles.radii > box_size_y / 2)] = (
        box_size_y / 2
        - particles.radii[np.where(y_new + particles.radii > box_size_y / 2)]
    )
    y_new[np.where(y_new - particles.radii < -box_size_y / 2)] = (
        -box_size_y / 2
        + particles.radii[np.where(y_new - particles.radii < -box_size_y / 2)]
...
def get_colliding_particles_unique_index_pairs(colliding_particles_indicies):
    ...
    """
    colliding_particles_all_index_pairs = np.column_stack(
        (colliding_particles_indicies[0], colliding_particles_indicies[1])
    )
    colliding_particles_unique_index_pairs = np.array(
        list(
            set(
                tuple(sorted(index_pair))
                for index_pair in colliding_particles_all_index_pairs
            )
        )
    )
...
def simulate(
    ...
    update_positions(
        particles, x_new, y_new, box_size_x, box_size_y, restitution_coef_bc
    )
    if frame_idx % 2 == 0:
        accelerate(particles, acceleration_vector)
    ...
    colliding_particles_indicies = np.where(
        (distance_matrix != 0) & (distance_matrix < particles.radii_sum)
    )
    ...
    ax.clear()
    circles = [
        plt.Circle((x_i, y_i), radius=r)
        for x_i, y_i, r in zip(particles.x, particles.y, particles.radii)
    ]
    collection = matplotlib.collections.PatchCollection(
        circles, cmap=matplotlib.cm.jet, alpha=0.8
    )
...
def simulate_compare(
    frame_idx,
    particles_list,
    acceleration_vector_list,
    ax_list,
    box_size_x,
    box_size_y,
    restitution_coef_bc_list,
    restitution_coef_pc_list,
    titles,
):
    for (
        particles,
...
def collision(
    ...
    """
    colliding_particles_unique_index_pairs = get_colliding_particles_unique_index_pairs(
        colliding_particles_indicies
    )
    ...
    if adjust_positions:
        ...
        distances = np.linalg.norm(
            positions[colliding_particles_1] - positions[colliding_particles_2], axis=1
        )
        norm_vector = (
            positions[colliding_particles_1] - positions[colliding_particles_2]
        ) / distances[:, None]
        shifts = (
            particles.radii[colliding_particles_1]
            + particles.radii[colliding_particles_2]
            - distances
        ) / 2
        adjusted_positions_1 = (
            positions[colliding_particles_1] + shifts[:, None] * norm_vector
        )
        adjusted_positions_2 = (
            positions[colliding_particles_2] - shifts[:, None] * norm_vector
        )
    ...
    impact_velocities_1 = np.sum(
        (velocities[colliding_particles_1] - velocities[colliding_particles_2])
        * (positions[colliding_particles_1] - positions[colliding_particles_2]),
        axis=1,
    )
    impact_velocities_1 = (
        ...
        * impact_velocities_1[:, None]
    )
    effective_mass_1 = (
        (1 + restitution_coef_pc)
        * particles.mass[colliding_particles_2]
        / (
            particles.mass[colliding_particles_1]
            + particles.mass[colliding_particles_2]
        )
    )
    delta_v_1 = effective_mass_1[:, None] * impact_velocities_1

    impact_velocities_2 = np.sum(
        (velocities[colliding_particles_2] - velocities[colliding_particles_1])
        * (positions[colliding_particles_2] - positions[colliding_particles_1]),
        axis=1,
    )
    impact_velocities_2 = (
        (positions[colliding_particles_2] - positions[colliding_particles_1])
    ...
    particles.v_x[colliding_particles_1] = (
        particles.v_x[colliding_particles_1] - delta_v_1.T[0]
    )
    particles.v_y[colliding_particles_1] = (
        particles.v_y[colliding_particles_1] - delta_v_1.T[1]
    )
    particles.v_x[colliding_particles_2] = (
        particles.v_x[colliding_particles_2] - delta_v_2.T[0]
    )
    particles.v_y[colliding_particles_2] = (
        particles.v_y[colliding_particles_2] - delta_v_2.T[1]
    )
