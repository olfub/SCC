    ...
    def massless_acceleration(self, objects_X, center_X):
        ...
        for j in range(n):
            for k in range(m):
                ...
                d = np.linalg.norm(v)
                a[j, :] = a[j, :] + self.gravitationCst * (
                    self.galaxies[k].centralMass * v
                ) / (d**3)
        return a
            ...
            for k in range(n):
                if j != k:
                    ...
                    f = self.galaxies[k].dynamicFriction(
                        d, center_V[j, :], center_V[k, :], self.galaxies[j].centralMass
                    )
                    a[j, :] = (
                        a[j, :]
                        + self.gravitationCst
...
class Galaxy_Collision:
    def __init__(self, galaxies, path):
        ...
        self.tags = np.concatenate(
            [
                np.full((self.galaxies[i].nbParticles,), i)
            ...
            with open(new_path, "w") as file:
                ...
                for i in range(self.n_galaxies):
                    ...
                    file.write(
                        gal
                        + "MASS : "
                        + str(self.galaxies[i].centralMass)
                        + " N"
                        + gal
                        + " : "
