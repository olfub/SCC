...
from external.GalaxyCollision.sources.common.functions import *
...
class MasslessGalaxy(ObjectCluster):
    def __init__(self, positions, velocities, centralMass, haloRadius):
        ...
        self.haloVelocity = 2 * np.sqrt(
            self.gravitationCst * self.centralMass / self.haloRadius
        )
...
class RingsMasslessGalaxy(MasslessGalaxy):
    def __init__(self, radii, particles, centralMass, haloRadius):
        ...
        super(RingsMasslessGalaxy, self).__init__(
            positions, velocities, centralMass, haloRadius
        )
